...
3. 기존 프로젝트 분석 결과를 바탕으로, `.agent/skills/` 하위의 모든 `SKILL.template.md` 파일들을 읽은 뒤, 현재 프로젝트 환경과 스택에 맞추어 내용(예: `{{FRAMEWORK}}` 변수 등)을 수정하여 원본 폴더에 `SKILL.md` 파일로 저장해줘.

4. 파일이 이미 존재하면 기존 내용을 보존하면서 병합해줘.
5. 불확실한 부분은 주석으로 표시하고, 확인이 필요하다고 알려줘.
6. 작업이 완료되면, 이 init.md 파일을 삭제해줘.
```

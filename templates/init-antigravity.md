...
3. 기술 스택과 컨벤션은 GEMINI.md에 정의했으므로 스킬 파일에 중복해서 넣지 마.
   대신, `.agent/skills/` 하위의 모든 `SKILL.template.md` 파일들을 그대로 읽어들인 뒤, 원본 폴더에 `SKILL.md` 파일로 저장해줘.

4. 파일이 이미 존재하면 기존 내용을 보존하면서 병합해줘.
5. 불확실한 부분은 주석으로 표시하고, 확인이 필요하다고 알려줘.
6. 작업이 완료되면, `.agent/skills/` 하위의 모든 `SKILL.template.md` 파일들과 이 init.md 파일을 모두 삭제해줘.
```

...
## Build & Quality Gate Checklist

### 빌드 & 컴파일
- [ ] 프로젝트 빌드/컴파일 에러 없음
- [ ] 문법 오류 없음
- [ ] TypeScript 타입 체크 통과 (해당 시)

### TDD 준수 확인
- [ ] 테스트 코드가 프로덕션 코드보다 먼저 작성되었는지 확인
- [ ] Red-Green-Refactor 사이클 준수
- [ ] 비즈니스 로직 유닛 테스트 커버리지 ≥ 80%
- [ ] 통합 테스트: 핵심 사용자 플로우 검증 완료
- [ ] 테스트 스위트 실행 시간 적정 (< 5분)

### 페이즈별 게이트
- [ ] 현재 페이즈의 모든 태스크 체크박스 완료
- [ ] 린트 에러 없음
- [ ] 기존 기능 회귀 없음
- [ ] 수동 테스트로 기능 동작 확인
- [ ] 코드 포맷팅 일관성 유지
...
- Use local search tools (`grep_search`) for code analysis patterns

...
- Use local code analysis commands (`list_dir`, `grep_search`) to map the codebase:
  - `list_dir` "src/": Understand structure
  - `grep_search` ("password.*=.*[\"']"): Find hardcoded secrets
  - `grep_search` ("execute.*\\$\\{"): Find SQL injection
  - `grep_search` ("innerHTML"): Find XSS vulnerabilities

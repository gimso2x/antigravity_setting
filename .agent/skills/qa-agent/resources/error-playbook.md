...
**Symptoms**: Cannot find file, `grep_search` or `list_dir` returns nothing

1. Check: correct file path? Use `grep_search` with broader terms
2. Check: is the code in a different directory or monorepo?
3. Use `list_dir` on parent directories to find the structure
...
2. Use pattern search to find high-risk areas:
   - `grep_search` ("password|secret|token|api_key")
   - `grep_search` ("execute|eval|innerHTML")
...
1. Retry once
2. If 2 consecutive failures: output results to standard output/chat
3. Add `fallback: true` flag to result

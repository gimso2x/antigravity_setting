...
[feature/docker] Dockerfile 빌드 단계 네트워크 진단 RUN 명령 추가
...
```powershell
git add <파일경로>; git commit -m "[현재브랜치명] 변경 내용 요약" -m "- 파일1: 변경 상세`n- 파일2: 변경 상세"
```

예시:

```powershell
git add docker/Dockerfile-development; git commit -m "[feature/docker] Dockerfile 빌드 단계 네트워크 진단 RUN 명령 추가" -m "- Dockerfile-development: 빌드 단계에 curl/nslookup으로 네트워크 진단 RUN 명령 추가`n- 내부 API 접근 가능 여부 확인 목적"
```
